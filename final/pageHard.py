...
class PageHard(tk.Frame):
    def __init__(self, parent, controller):
        self.sysFileName,self.sysText,self.spkrFileName,self.spkrText = self.getNum("HARD")
    ...
    def stopRecordVoice(self):
        ...
        p.wait()
        _,a = read("../results/"+self.spkrFileName+".wav")
    ...
    def submit(self):
        ...
        if score<0.01:
            ...
            # myframe = self.__name__
            frame.plot(self.spkrFileName, "PageHard")
    ...
    def next_level(self):
        if self.level - self.startlevel == 4:
            ...
            self.sysFileName, self.sysText, self.spkrFileName, self.spkrText = self.getNum("HARD", True)
            self.plot(self.graphFrame)
            self.syslabel.config(text = self.sysText)
            self.spkrlabel.config(text = self.spkrText)
            self.controller.show_frame("PageHard")
            return True
